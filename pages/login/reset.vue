		...
		<view class="form_input_css " :loading="loading">
			<form>
				...
				<view class="cu-form-group">
					<input placeholder="设置新密码" password v-model="myFormData.newPassword"></input>
				</view>
				<view class=" text-center margin-top">
					<button type="primary" style="width: 100%;;border-radius: 50px;min-height: 52px;margin-top: 50px;"
						class="cu-btn bg-blue lg shadow" @click="onSubmit">重置完成
					</button>
				</view>
			</form>
		</view>
	...
	export default {
		...
		methods: {
			...
			onSubmit() {
				...
				if (!myForm.newPassword || myForm.newPassword.length == 0) {
					uni.showModal({
						content: "请输入密码",
						showCancel: false,
					});
				...
				if (myForm.account.newPassword < 5) {
					uni.showModal({
						content: "密码长度必须大于6位",
						showCancel: false,
					});
				...
				this.loading = true;
				this.$http.request({
					url: '/userForgetPassWordByPhone',
					method: 'POST',
					dataType: 'text',
					data: {
						phone: myForm.phone,
						code: myForm.verificationCode,
						password: myForm.newPassword,

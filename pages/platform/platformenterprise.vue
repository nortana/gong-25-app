				...
				<scroll-view :scroll-top="scrollTop" scroll-y="true" :style="[{height:clineHeight+'px'}]"
					...
					<block v-for="(item,index) in list" :key="index">
						<view class="" style="
							...
							<view class="bg-shadeTop" style="height: 25px;border-bottom: 1px solid;position: relative;">
							...
							<view style="display: flex;flex-direction:row;margin-top: 5px;position: relative;">
								<view class="" style="font-size: 13px;padding-left: 10px">{{item.work_name}}</view>
								...
								<view style="border: 1px solid;line-height: 12px;margin-left: 5px;padding: 1px;
														...
														margin-top: 2px;">距您{{item.longitude_latitude_distance}}KM</view>
								
								<!-- "salary_type": 1, //薪资类别：1=时薪，2=日薪，3=周薪，4=月薪，5=季薪，6=年薪 -->
								<view v-if="item.salary_type==1" class="class-salary-type">时薪</view>
								<view v-if="item.salary_type==2" class="class-salary-type">日薪</view>
								<view v-if="item.salary_type==3" class="class-salary-type">周薪</view>
								<view v-if="item.salary_type==4" class="class-salary-type">月薪</view>
								<view v-if="item.salary_type==5" class="class-salary-type">季薪</view>
								<view v-if="item.salary_type==6" class="class-salary-type">年薪</view>
								<view style="font-size: 13px;padding-left: 10px;position: absolute;
							...
							<view class="" style="padding: 0 10px;font-size: 13px;">
								{{item.work_require}}
							</view>

							<view class="bg-shadeTop"
								style="padding-left: 5px;height: 20px;line-height20px;margin:2px 10px;font-size: 12px;line-height: 20px;">
								<text>15人速配中</text>
							</view>
			...
			getClineHeight() {
				const res = uni.getSystemInfo({
					success: (res => {
						...
						this.clineHeight = res.windowHeight - this.getBarHeight() - 235;
					})
				});
